...
public readonly struct IPv6Address : IIPAddress<IPv6Address, UInt128>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryFormat(Span<char> destination, out int charsWritten) => IPv6AddressFormatter.TryFormat(Bytes, destination, out charsWritten);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool TryParse(ReadOnlySpan<char> source, out IPv6Address result)
    {
        Span<byte> ipAddressBytes = stackalloc byte[Unsafe.SizeOf<IPv6Address>()];
        if (IPv6AddressFormatter.TryParse(source, ipAddressBytes))
        {
            result = new IPv6Address(ipAddressBytes);
            return true;
        }
        result = default;
        return false;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static IPv6Address Parse(ReadOnlySpan<char> source) => FormattingHelper.Parse<IPv6Address>(source);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static IPv6Address Parse(string source) => FormattingHelper.Parse<IPv6Address>(source);
